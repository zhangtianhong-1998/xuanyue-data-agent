...
class ScopeExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue):
        ...
        async def double(seed: int):
            ...
            if payload.get("mode") == "slow":
                ...
                try:
                    ...
                    self.record("tool_cancelled", task_id=context.task_id)
                    raise
        ...
        try:
            ...
            raise
        # AgentScope may absorb cancellation and finish its stream. Do not publish that as success.
        if context.task_id in self.cancellation_requested:
            self.record("execute_cancel_acknowledged", task_id=context.task_id)
            return
    ...
    async def cancel(self, context, event_queue):
        self.cancellation_requested.add(context.task_id)

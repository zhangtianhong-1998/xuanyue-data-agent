...
def run(output):
    ...
    results = {"experiment": "runtime-interoperability-v1", "date": datetime.now(timezone.utc).isoformat(),
        ...
        "installed_versions": {environment: json.loads(subprocess.check_output([
            str(py(environment)), "-c", "import importlib.metadata as m,json; print(json.dumps({n:m.version(n) for n in "
                + repr(["langgraph", "langgraph-checkpoint-sqlite", "a2a-sdk"] if environment == "parent" else ["agentscope", "a2a-sdk", "uvicorn", "mcp"])
                + "}))"], text=True)) for environment in ["parent", "child"]},
    ...
    with tempfile.TemporaryDirectory(prefix="xuanyue-interop-") as temporary:
        ...
        try:
            ...
            check("I08", "cancel interrupts executing cooperative AgentScope tool",
                ...
                and len(task_events(cancel_id, "tool_cancelled")) == 1
                and len(task_events(cancel_id, "execute_cancel_acknowledged")) == 1
                and not task_events(cancel_id, "tool_completed"),
                "a2a_sdk_cancellation_plus_application_cancel_handler",
                {"tool_cancelled_count": len(task_events(cancel_id, "tool_cancelled")),
                 "executor_acknowledgement_count": len(task_events(cancel_id, "execute_cancel_acknowledged")),
                 "tool_completion_count": len(task_events(cancel_id, "tool_completed")), "task_id": cancel_id})
            ...
            negative = worker("mapping", "mapping-negative")
            check("I10", "invalid structured data and unsupported modality rejected",
                negative["rejected"] == ["invalid_schema", "unknown_modality"],
                "application_schema_and_capability_gate", negative)
            ...
            results["summary"] = {"passed": sum(c["status"] == "passed" for c in results["checks"]),
                "failed": sum(c["status"] == "failed" for c in results["checks"]), "planned_checks": 10,
                "not_reached": 10 - len(results["checks"]), "all_passed": len(results["checks"]) == 10 and not results["failures"]}

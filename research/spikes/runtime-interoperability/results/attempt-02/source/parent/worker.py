...
def run(args):
    ...
    def normalize(state):
        ...
        artifacts = [{"artifact_id": a["artifactId"], "parts": [
            {"kind": "text", "text": part["text"]} if "text" in part else
            {"kind": "structured", "value": part["data"]} for part in a["parts"]]} for a in task["artifacts"]]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--action", choices=["run", "crash", "recover", "pause", "resume-input", "query", "slow", "cancel", "fork"], default="run")

...
def start(store, run, seed=3, source=None):
    ...
    state["route"] = route("parallel")  # Model simulator, not a real small model.
    store.event(run, "route", "completed", payload={"provider": "simulator", "route": state["route"]})
...
def suite():
    ...
    with tempfile.TemporaryDirectory(prefix="xuanyue-handwritten-") as temp:
        ...
        try:
            ...
            checks["F08"] = bool(events) and all({"event_id", "run_id", "node_id", "attempt", "branch_id", "phase"} <= e.keys() for e in events)

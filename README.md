...
**当前在需求和技术路线评审阶段，尚无完整客户端。** 用户可以直接发起自主 Agent 任务，选择主智能体及其内核；LangGraph 和 AgentScope 都要能承担这个角色。Skill 供 Agent 按需阅读和判断。用户也可以编排规则更固定的工作流；它能独立运行，不一定需要主智能体。Agent 自主运行后调用工作流属于后期候选。可插拔接口和各项能力的实现方式仍需逐步验证。

...
from .llm import Hint, LLM, Message, ModelReply, ModelRequest, Text, ToolCall, ToolResult, ToolSpec
        ...
        if isinstance(block, TextBlock):
            ...
            if isinstance(output, list):
                if len(output) != 1 or not isinstance(output[0], TextBlock):
                    raise UnsupportedModelContent("tool result with multiple or non-text blocks")
            ...
            raise UnsupportedModelContent(f"unsupported message block: {type(block).__name__}")
...
class _PortModel(ChatModelBase):
    def __init__(self, model: str, llm: LLM, allowed_tools: Sequence[ToolSpec]) -> None:
        super().__init__(CredentialBase(), model, self.Parameters(), stream=False, max_retries=0)
    ...
    async def _call_api(
        ...
        **kwargs: Any,
        ...
        if kwargs or (tool_choice is not None and (tool_choice.mode not in ("auto", "none") or tool_choice.tools is not None)):
            raise UnsupportedModelContent("this tool choice or model option is not supported in this slice")
        ...
        if any(self._allowed_tools.get(spec.name) != spec for spec in specs):
            raise UnsupportedModelContent("kernel requested a tool absent from the product registry")
        reply = await self._llm.complete(
            ModelRequest(model_name, tuple(_model_message(msg) for msg in messages), specs, tool_mode),
        )
        if not isinstance(reply, ModelReply):
            raise TypeError("LLM.complete must return ModelReply")
        blocks = []
        for part in reply.parts:
            if isinstance(part, Text):
                ...
                if tool_mode == "none":
                    raise UnsupportedModelContent("model called a tool after tools were disabled")
                if part.name not in {spec.name for spec in specs}:
                    raise UnsupportedModelContent(f"model called unavailable tool {part.name!r}")
                blocks.append(ToolCallBlock(id=part.id, name=part.name, input=part.arguments))
            else:
                raise UnsupportedModelContent(f"unsupported model reply part: {type(part).__name__}")
        return ChatResponse(content=blocks, is_last=True)


def _native_tool(tool: ReadOnlyTool, task: Task) -> FunctionTool:
    async def invoke(**kwargs: object) -> ToolChunk:
        result = await tool.invoke(task, kwargs)
        if not isinstance(result, str):
            raise TypeError(f"tool {tool.spec.name!r} must return text")
...
_PUBLIC_EVENTS: Mapping[str, tuple[str, frozenset[str]]] = {
    ...
    "TOOL_RESULT_START": ("tool_result_started", frozenset({"tool_call_id", "tool_call_name"})),
...
def project_native_event(event: object) -> tuple[str, Mapping[str, object]] | None:
    ...
    if native_type in _SKIPPED_EVENTS:
        return None
    public_spec = _PUBLIC_EVENTS.get(native_type) if isinstance(native_type, str) else None
    if public_spec is None:
        safe_type = native_type if isinstance(native_type, str) and _SAFE_EVENT_TYPE.fullmatch(native_type) else "unrecognized"
...
class AgentScopeKernel:
    id = "agentscope"

    def __init__(self, llm: LLM, tools: Sequence[ReadOnlyTool], system_prompt: str) -> None:
    ...
    async def stream(self, task: Task) -> AsyncIterator[Event]:
        ...
        root = Agent(
            "primary-agent",
            self._system_prompt,
            model=_PortModel(task.model, self._llm, [tool.spec for tool in self._tools]),
            toolkit=Toolkit(tools=[_native_tool(tool, task) for tool in self._tools]),

...
from xuanyue import Event, KernelUnavailable, Runtime, Task
from xuanyue.agentscope import AgentScopeKernel, UnsupportedModelContent, _model_message, project_native_event
from xuanyue.llm import Hint, Message, ModelReply, ModelRequest, Text, ToolCall, ToolResult, ToolSpec
from xuanyue.tools import ReadOnlyTool


SCHEMA = {
    "type": "object",
    "properties": {"orders": {"type": "integer"}, "units_per_order": {"type": "integer"}},
...
class ScriptedLLM:
    ...
    async def complete(self, request: ModelRequest) -> ModelReply:
        ...
        if len(self.requests) == 1:
            return ModelReply((Text("先核对。"), ToolCall("calc-1", "multiply", '{"orders":21,"units_per_order":2}')))
...
class RuntimeTests(unittest.IsolatedAsyncioTestCase):
    async def test_agentscope_root_uses_product_model_and_tool_ports(self) -> None:
        ...
        runtime = Runtime([AgentScopeKernel(model, [ReadOnlyTool(SPEC, authorize, execute)], "Use the tool to check the calculation.")])
        task = Task("run-1", "agentscope", "synthetic-small", "合成订单有 21 单，每单 2 件。请核对总件数。")
        ...
        self.assertEqual(authorized, [(task.run_id, {"orders": 21, "units_per_order": 2})])
        ...
        self.assertIn("42", "".join(str(event.payload.get("delta", "")) for event in events if event.kind == "text_delta"))
        self.assertTrue(any(event.kind == "tool_result_finished" and event.payload.get("state") == "success" for event in events))
        self.assertTrue(any(event.kind == "reply_finished" and event.payload.get("finished_reason") == "completed" for event in events))
        self.assertFalse(any(event.kind == "coverage_gap" for event in events))
        self.assertTrue(all(event.payload.get("usage_status") == "unknown" for event in events if event.kind == "model_call_finished"))
        second_messages = model.requests[1].messages
        assistant_parts = [part for message in second_messages if message.role == "assistant" for part in message.parts]
        self.assertTrue(any(isinstance(part, ToolCall) for part in assistant_parts))
        self.assertTrue(any(isinstance(part, ToolResult) and part.output == "42" and part.state == "success" for part in assistant_parts))
    ...
    async def test_kernel_selection_is_exact(self) -> None:
        ...
        model = ScriptedLLM()
        runtime = Runtime([AgentScopeKernel(model, [], "Answer directly."), FakeKernel()])
        task = Task("run-3", "another-kernel", "synthetic-small", "test")
        self.assertEqual([event.kind async for event in runtime.stream(task)], ["reply_finished"])
        with self.assertRaises(KernelUnavailable):
            runtime.stream(Task("run-4", "langgraph", "synthetic-small", "test"))
        self.assertEqual(model.requests, [])

    async def test_model_receives_no_tool_choice_when_agent_hits_iteration_limit(self) -> None:
        class RepeatedToolLLM:
            def __init__(self) -> None:
                self.requests: list[ModelRequest] = []
            ...
            async def complete(self, request: ModelRequest) -> ModelReply:
                ...
                return ModelReply((ToolCall(f"call-{len(self.requests)}", "multiply", '{"orders":1,"units_per_order":1}'),))
        ...
        runtime = Runtime([AgentScopeKernel(model, [ReadOnlyTool(SPEC, authorize, execute)], "Use the tool.")])
        events = [event async for event in runtime.stream(Task("run-limit", "agentscope", "synthetic-small", "Keep checking."))]
        self.assertEqual(model.requests[-1].tool_mode, "none")
        self.assertTrue(any(isinstance(part, Hint) for message in model.requests[-1].messages for part in message.parts))
    ...
    async def test_tool_schema_requires_object_properties(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "object properties"):
            ReadOnlyTool(ToolSpec("empty", "No args", {"type": "object", "additionalProperties": False}), authorize, execute)
    ...
    def test_hidden_and_unknown_native_content_are_handled_explicitly(self) -> None:
        ...
        self.assertEqual(project_native_event(FutureEvent()), ("coverage_gap", {"source": "agentscope", "native_type": "FUTURE_EVENT"}))
        native = SimpleNamespace(role="assistant", content=[ThinkingBlock(thinking="DO_NOT_PERSIST")])
        message = _model_message(native)
        self.assertEqual(message, Message("assistant", (), private_content_omitted=True))

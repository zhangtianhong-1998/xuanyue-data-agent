...
当前是需求和架构评审阶段，尚无完整客户端。已确认的要求是：LangGraph 和 AgentScope 都能担任主智能体，不同内核可组成父子任务，今后还能接入更多内核。接入方式、工作流由谁调度，以及各项能力怎样实现，仍在验证。

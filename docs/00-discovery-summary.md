...
当前是需求和架构评审阶段，尚无完整客户端。已确认的要求是：用户在编排工作流时指定主智能体及其内核，LangGraph 和 AgentScope 都能承担这个角色，不同内核可组成父子任务，今后还能接入更多内核。接入方式、工作流由谁调度，以及各项能力怎样实现，仍在验证。
...
| 词语 | 在本项目中的意思 |
| --- | --- |
| 主智能体 | 一次任务的负责人：接收目标、推进计划、委派子任务并整理结果。由用户编排流程时指定；LangGraph 和 AgentScope 都要能运行这个角色。 |
| 内核 | 负责运行 Agent 的框架，例如 LangGraph 或 AgentScope。内核选择与模型选择是两件事。 |
| 转接口 | 把产品约定的任务、消息、工具调用和事件接到某个内核的代码。换内核时，界面和历史仍读产品保存的记录。具体接口待评审。 |
| 工作流 | 用户保存的步骤、分支及主智能体选择。它如何调度、暂停和恢复，是仍需实验的问题。 |

...
用户直接创建自主任务，选择 AgentScope 为主内核；旧的 LangGraph 任务仍可读。AgentScope 委派 LangGraph 子任务时，双方都通过产品保存父子关系、授权、产物和公开轨迹。独立确定性工作流可有自己的节点与内核配置，纯固定流程无须主 Agent。这个例子在[整体架构](01-candidate-architecture.md)展开。本页按问题查阅：转接口看第 2 节；A2A 与委派看第 4–5 节；换内核与用户界面看第 6–7 节；验收看第 8 节。
...
| 编排独立确定性工作流 | 用户保存固定步骤；Agent 节点的职责与内核随版本保存，纯固定流程无须主 Agent | 校验每个节点的能力；修改绑定须保存新流程版本 |
...
[DeepSeek Harness 的固定源码](../research/11-deepseek-harness-pluggability.md)提供服务插件、按接口调用和配置替换的例子。它的主 Agent 工厂与工作流服务在同一 Context 中各只有一个实例；**按每次自主任务的用户选择绑定不同主内核是本项目需要另做的部分**。注册表记录可用内核；任务开始时固定 kernel_id、适配器版本与能力快照。含 Agent 节点的工作流也保存相应绑定；旧运行保留原绑定。
...
1. 用户创建自主任务时，明确指定顶层主智能体及其 LangGraph 或 AgentScope 内核；产品校验并解析为 PrimaryAgentRuntime。子 Agent 引用稳定 agent_id 与 ExecutionProfile，再解析为各自固定的 RuntimeBinding。若是独立确定性工作流，按其保存的 Agent 节点配置解析；纯固定流程无须主 Agent。模型选择与内核选择是两项配置，不能混为同一个路由。
...
用户创建新自主任务时可以改选主内核；修改已保存流程中的 Agent 节点配置须另存版本。旧版本及在途运行继续绑定旧内核。升级/卸载前提示依赖它的未完成任务，并按保留、停止或交接策略处理。安全策略撤销后即使有旧 runtime 也不能恢复被禁止的动作。
...
自主任务创建页让用户指定主智能体及其内核；任务页展示实际绑定、计划、进度、待答问题、产物和分支。工作流编辑页另外配置 Agent 节点，纯固定流程不显示不存在的主 Agent。运行时不能悄悄替换用户的选择。普通使用者无需操作框架对象或 A2A taskId。

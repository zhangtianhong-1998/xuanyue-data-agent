...
不支持这种契约的外部服务，提交超时后进入核对状态；不得自动重发有副作用的任务。已保存 taskId 的恢复查询，与首次提交未知是不同测试场景。业务副作用仍使用[工作流的 EffectIntent 规则](04-workflow-and-trace.md#3-点击旧节点后可以做什么)。
...
交接快照传递业务状态和允许使用的上下文，不包含密钥、隐藏推理或伪造的原生检查点。目标内核重新校验工具、模型、授权、输入哈希、画像版本和预算，产生新 run/branch 并关联来源；若产品支持，可以归属派生的新会话，或让当前会话的活动头指向新分支。两者都不表示异构原生检查点原位恢复。结果可不同，记录转换损失。若用户要求从任意内核任意内部节点热迁移，这超出本轮可保证范围，需逐个编写并验证迁移器。
...
从轨迹点击旧节点时，先说明它是否可恢复及实际粒度，再提供派生新会话或在当前会话改指活动头的选项。原内核节点分叉、重新执行整个子任务、从可移植交接点建立后续运行是不同的执行能力；界面不能都写成“从此节点恢复”。用户应知道会重算哪些步骤、是否再次收费和可能产生哪些副作用。两种会话模式见[运行轨迹设计](04-workflow-and-trace.md#2-agent-运行时怎样从旧节点分叉)。

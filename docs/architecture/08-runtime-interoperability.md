...
用户在编排工作流时指定 AgentScope 为主智能体并保存版本；运行时产品照此绑定，旧的 LangGraph 任务仍可读。AgentScope 委派 LangGraph 子任务时，双方都通过产品保存父子关系、授权、产物和公开轨迹。这个例子在[整体架构](01-candidate-architecture.md)展开。本页按问题查阅：转接口看第 2 节；A2A 与委派看第 4–5 节；换内核与用户界面看第 6–7 节；验收看第 8 节。
...
| 操作 | 面向用户的目标 | 产品的处理方式 |
| --- | --- | --- |
| 编排或修改工作流的主智能体 | 用户指定 LangGraph 或 AgentScope，选择随工作流版本保存；历史入口一致 | 运行前校验用户选定的绑定；不满足必需能力就说明并停止，不自动换内核；改选须保存新版本 |
...
[DeepSeek Harness 的固定源码](../research/11-deepseek-harness-pluggability.md)提供服务插件、按接口调用和配置替换的例子。它的主 Agent 工厂与工作流服务在同一 Context 中各只有一个实例；**按用户的工作流配置绑定不同主内核是本项目需要另做的部分**。注册表记录可用内核；工作流版本保存用户选择，运行时固定 kernel_id、适配器版本与能力快照。新流程版本可以指定不同主内核，旧运行保留原绑定。
...
K1 的共同验收必须包含 LangGraph、AgentScope 两种 PrimaryAgentRuntime 绑定：用结构相同、由用户分别指定主智能体的两个工作流版本，通过 Workflow Durable 的产品可见检查。工作流可以由独立产品层调度，也可以由所选框架的适配器调度；此项取舍要在 K0 双主实验后评审。**仅有两种子任务执行器或单向父子委派，不满足双主需求。** AgentScope 目前未通过历史节点分叉的发布包实验；须补机制并验收，不能以能力预检失败作为最终交付。
...
1. 用户编排并保存工作流时，明确指定顶层主智能体及其 LangGraph 或 AgentScope 内核；运行该版本时产品校验并解析为 PrimaryAgentRuntime。流程节点和子 Agent 引用稳定 agent_id 与 ExecutionProfile，再解析为各自固定的 RuntimeBinding。模型选择与内核选择是两项配置，不能混为同一个路由。
...
用户修改主智能体并保存新工作流版本后，新运行才使用新绑定；旧版本及在途运行继续绑定旧内核。升级/卸载前提示依赖它的未完成任务，并按保留、停止或交接策略处理。安全策略撤销后即使有旧 runtime 也不能恢复被禁止的动作。
...
工作流编辑页让用户指定主智能体及其内核，也能为子 Agent 选择内核；任务页展示该版本实际绑定的主智能体、计划、进度、待答问题、产物和分支。运行时不能悄悄替换用户的选择。没有保存工作流的临时任务，可在创建任务时由用户选择主智能体；这不改变已保存流程的规则。普通使用者无需操作框架对象或 A2A taskId。

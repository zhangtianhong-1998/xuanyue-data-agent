# 多内核适配、父子 Agent 与 A2A

[返回设计入口](../00-discovery-summary.md) · 上一步：[核心对象](03-core-contracts.md) · 下一步：[工作流与轨迹](04-workflow-and-trace.md)

日期：2026-09-25；状态：新增需求已记录，以下方案待评审。本页是多内核绑定、通信、切换与兼容规则的唯一详细说明；对象字段仍在[核心对象](03-core-contracts.md)。协议与上游事实见[A2A 证据](../research/10-a2a-interoperability.md)。

## 1. 需求改变了哪项决定

上一版“只接一个 RuntimeAdapter、不同时用两套内核”的限制撤回。**建议改为：产品协议稳定，多种内核可注册；LangGraph 先作为默认流程编排器，AgentScope 可以作为子 Agent 执行器。** 一个任务的每一层执行仍有明确负责人，不能让两套框架同时接管同一个节点的重试和状态。

需要分别支持三种操作：

| 操作 | 面向用户的目标 | 产品的处理方式 |
| --- | --- | --- |
| 为新任务切换内核 | 对话、流程、产物与历史入口不变 | 选择满足 ExecutionProfile 的绑定，运行开始后固定版本 |
| 父子使用不同内核 | 用户仍看到一个父任务和可展开的子任务 | 通过统一委派服务提交，记录父子身份；各内核管理自己的局部状态 |
| 从旧进度换内核继续 | 已完成的业务结果尽量保留 | 在声明可移植的交接点生成新运行；原内核 checkpoint 不强行翻译 |

“无感”承诺的是正常使用路径和数据呈现的一致性。回答内容、速度、费用、可回溯粒度与取消能力不一定相同；涉及能力不足、出网目的地变化或不可安全迁移时，必须展示具体影响。不能为了隐藏技术差异而伪报成功。
...
flowchart TD
  ...
  C --> W[WorkflowRuntime 接口]
  W --> LG[LangGraph 默认编排器]
  LG --> D[委派服务：绑定、任务账本、恢复核对]
  D --> T[AgentTransport：本地 IPC 或 A2A]
  T --> E[AgentTaskRuntime 接口]
  E --> AS[AgentScope 执行器]
  E --> LE[LangGraph Agent 执行器]
  E --> O[后续其他内核]
...
| 接口 | 提供什么 | 不能假定什么 |
| --- | --- | --- |
| WorkflowRuntime | validate/compile/start、状态观察、取消请求；按能力提供 checkpoint/resume/history/fork | 所有 Agent 框架都能编译任意用户流程、支持同样回溯粒度 |
...
三者都是产品内部接口名，尚未实现正式 SDK。一个框架适配器可以实现两个 runtime 接口，也可以只实现 AgentTaskRuntime。`RuntimeBinding` 决定某次任务具体使用哪个实现。UI、业务工具和存储只依赖产品契约，不持有 LangGraph StateGraph 或 AgentScope AgentState。
...
**不要只统一最低能力。** 必需操作形成基本契约，历史分叉、内部节点事件、硬取消等作为可探测扩展。工作流声明自己的必要能力，路由器据此选择执行器；没有合格执行器则解释缺口，不能静默砍掉用户需要的步骤。
...
| Workflow Durable | 进一步通过固定流程编译、检查点恢复、声明粒度的分叉/版本检查 | 可恢复工作流的默认编排器 |
...
兼容预检输出三类结果：可执行；需明确的转换/降级；不兼容。例：只接受文字的执行器不能无提示丢图片；不支持 fork 的子任务可以从委派边界重新执行为新任务，但按钮必须写“重新执行子任务”，不能写“恢复子任务内部节点”。要求完整内部轨迹的流程不能自动改用仅提供最终结果的外部 Agent。

K1 的建议验收是一个完整 WorkflowRuntime（先 LangGraph），以及 LangGraph/AgentScope 两种 AgentTaskRuntime 的受管理任务档位。**不要求 AgentScope 首版就编译整套工作流；也不能把“两种子任务执行器”说成“两个完整流程引擎已可互换”。** 后续新增编排器，需独立通过 Workflow Durable 用例。

## 4. A2A 用在什么边界

建议跨进程的父子 Agent 使用 A2A 作为标准通信选择。K1 先固定一种 binding 做真实互通；同进程可用更轻的调用方式，但必须通过同一产品任务契约，不能绕过权限、预算和事件服务。A2A consumer 接入外部 Agent；A2A provider wrapper 把受管理的本地执行器暴露成任务服务，两条方向分别验收。
...
## 5. 父 LangGraph、子 AgentScope 怎样运行

1. 流程节点引用稳定 agent_id 和 ExecutionProfile；运行前解析为固定 RuntimeBinding。模型选择与内核选择是两项配置，不能混为同一个路由。
2. 委派服务先创建 DelegationRecord，预留父预算，记录参数哈希、父 attempt、工具与数据授权的子集，再提交。子执行器运行在独立环境，避免 AgentScope/MCP 与其他 SDK 依赖冲突；独立进程本身仍不是执行沙盒。
3. 受管理的 A2A wrapper 校验调用主体与任务范围，创建子任务，映射 Task、Message、Artifact 和公开事件。AgentScope 只管理子任务内部循环；父编排器只管理等待、合并和后续流程。谁拥有哪一层的重试写入策略，不让两层各自无界重试。
4. 收到 input-required 时，父任务展示同一套补充信息界面；用户回答送回已绑定子任务。父流程暂停不重复创建子任务。
5. 父进程退出后，从持久账本找回已存在的子 task，先 GetTask/重新订阅并核对，再决定继续等待、补输入或收集结果。子服务不可达显示状态未知，不能当作没有运行。
6. 子任务完成后，验收产物、结算预算，写父节点结果。项目及全局画像都通过产品 MemoryStore 提交候选，失败、取消和未选定分支不直接写入；子 Agent 不能绕过更新规则或扩大工具权限。外部服务自己的记忆保留与删除政策另外核验，产品不替其承诺可遗忘。
...
默认切换对新运行生效。在途运行继续绑定旧内核；升级/卸载前提示依赖它的未完成任务，并按保留、停止或交接策略处理。安全策略撤销后即使有旧 runtime 也不能恢复被禁止的动作。
...
默认任务页继续展示统一的计划、进度、待答问题、产物和分支。高级设置可以选择默认内核或每个 Agent 的绑定；普通使用者无需操作框架对象或 A2A taskId。
...
本轮已完成固定版本协议研究与 10 项合成互通检查；结果见[实验 README](../../research/spikes/runtime-interoperability/README.md)。**局部实验不表示下面整套产品方案已经实现。**

| 阶段 | 验收目标 |
| --- | --- |
| K1a | 注册/能力预检；LangGraph 编排器；至少两种受管理 Agent 执行器；父 LG→子 AS 的真实 A2A 调用；状态、身份、结构化产物与取消/恢复边界 |
| K1b | 在相同界面切换子 Agent 执行器；历史继续可读；等待输入、重连、比较新尝试；权限/预算/画像不随切换失效 |
| 后续 | 第二种完整 WorkflowRuntime；更多 A2A binding/版本；通过协议、能力和信任预检的外部 A2A 服务接入；特定内核之间有版本的状态迁移器 |

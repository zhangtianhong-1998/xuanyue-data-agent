...
三种运行都应有公开执行记录。只有运行已保存的工作流时，才存在相应的流程定义图；自主任务也应能查看调用、产物、失败和子任务。本页第 1 节讨论 Skill 与固定流程，第 2–4 节讨论 Agent 运行时的回溯、分支与界面，第 5 节列待验证事项。
...
用户给出的图片表达的是一条 **Agent 运行轨迹**：运行到后面时，用户可以选前面某个可恢复节点，修改输入或要求，再从那里生成另一段后续。它不是“流程定义图、本次执行图、尝试分支树”三张图。预先编排的工作流画布是另一种界面，不应拿来解释这张图片。
...
| 操作 | 会话里看到什么 | 旧后续怎样处理 |
| --- | --- | --- |
| 派生新会话 | 从选定节点打开新会话，沿新路径继续；原会话不变 | 留在原会话；新会话只引用分叉点之前获准继承的状态和产物 |
| 在当前会话改写后续 | 保持当前会话入口，把活动路径改到新分支；下次给 Agent 的上下文沿新路径构建 | 从默认时间线移开，但保留可找回的旧头和记录，用户可切回 |
...
“查看”“回放”“重试”“恢复”“从此处派生”是不同操作。尤其要区分：**回放只是看旧记录；从旧节点开始新执行会重做受影响的步骤，可能再次产生费用或外部操作。**
...
| 从此处派生新会话 | 从已验证可恢复的节点建立新会话和执行分支 | 原会话保持；不自动复制全部文件或权限 |
| 在当前会话改写后续 | 从该节点建新分支，把会话活动头改指新分支；保留旧头以便切回 | 只改变默认可见和继续使用的历史，不抹除已发生事件或副作用 |
| 切回旧分支 | 把活动头重新指向保留的旧头，并记录这次切换 | 不重新执行旧步骤；已撤销的授权不会因此恢复 |

开始分叉前先取得稳定检查点；正在写入的分支需暂停或选择已完成检查点。只展示有足够恢复信息的节点；如果只能根据公开消息和产物建立后续任务，应标为“从此处新建”，不冒充原生检查点恢复。输入、工具、模型或流程版本改变后，依赖它的下游结果失效；只有依赖完全相同的不可变产物才可显式复用。若用户修改检查点之前的输入，应回到该输入的第一个消费者之前重算，不能只改状态中的一个数字而沿用旧派生结果。

新分支固定 workflow/model/tool/skill/profile 和 runtime binding 的版本引用。跨内核切换及远端任务的重跑粒度见[多内核设计](08-runtime-interoperability.md#6-内核切换与状态可移植性)。节点恢复以框架可用的检查点粒度为准；不承诺能从任意一行代码或任何 token 中间恢复。LangGraph 子图的内部回溯粒度还取决于检查点配置，尚需单独验证。[官方 time-travel 说明](https://docs.langchain.com/oss/python/langgraph/use-time-travel)
...
主界面建议：左侧任务；中间对话；右侧默认产物，可切换“流程 / 轨迹”。轨迹面板以运行时间线及其分支为中心，标出当前活动头、派生的新会话和可切回的旧头；选中节点后可查看详情或比较两段后续，不要求用户在聊天记录里寻找每次工具调用。
...
5. **轨迹回溯与分支：**自主任务和固定流程都保留公开输入、工具、产物、异常和版本；在各自已验证的节点分别尝试派生新会话、当前会话活动头改指和切回旧头。检查原事件不变、后续重新执行的范围、在途操作及副作用提示。活跃分支数量和实际磁盘占用另测。

...
修订：2026-09-26；状态：待评审。本页描述希望产品怎样运行用户保存的工作流，以及用户怎样查看、重试和比较运行结果。画布和完整轨迹界面尚未实现；对象字段见[核心对象](03-core-contracts.md)。

## 先看懂这页

假设用户上传一份经营数据，想找出收入下降的原因。他可以保存一套步骤：程序先检查数据；小模型判断该走哪条分析路径；两个 Agent 分别分析地区和产品；最后由用户检查结论。保存时，用户还要指定**由哪个 Agent 担任主智能体、使用哪个内核**。下次运行同一版本，产品照这个选择执行，不自行换成另一种主内核。

运行过程中，用户应能看到每步用了什么输入、调用了什么工具、得到了什么结果。如果地区分析的结果不可信，用户可以回到一个已经保存的节点，改参数后产生一次**新尝试**。原尝试和新尝试都保留，用户比较后选择一个结果。这里的“回到节点”不表示能从模型输出的任意一个字、或工具执行到一半的位置继续。

本页分别说明三件事：第 1–2 节是**怎样设计步骤和选择模型**；第 3–4 节是**怎样记录运行和从历史节点再试**；第 5 节是**界面展示哪些记录**。第 6 节说明以后怎样验证 LangGraph 和 AgentScope 都能担任主智能体。以上仍是设计目标，不代表功能已完成。

## 1. 用户如何固化成熟逻辑

画布保存的是“下次怎样运行”的步骤、连线和设置，不是某次运行到一半的状态。用户可从模板开始，连接节点、指定主智能体及其内核、设置参数和模型，再保存版本；Agent 也可以提出草案，但须经用户确认。运行前，产品检查用户选的内核能否执行这套流程；不能执行就解释原因。改选主智能体要保存新版本，旧运行仍指向旧版本。由产品还是所选内核负责调度节点，仍待[双主路线实验](02-kernel-selection.md#3-仍需比较的两条工作流路线)决定；用户指定主智能体的规则不受这一选择影响。
...
它们回答不同的问题：**流程定义图**是事先保存的计划；**本次执行图**记录这次实际走过哪些步骤；**尝试分支树**展示用户从哪个旧结果重新尝试，以及最后选中了哪个结果。改计划不会改写旧运行；从旧节点再试也不会覆盖原结果。
...
下图对应用户提供的“多条尝试分别通过或拒绝”的示意。这里的分叉表示**从同一个已保存状态出发，分别尝试不同参数或模型**；同时运行的两个子 Agent 应另行标明，避免误认为它们是三次重试。
...
1. 固定图：用结构相同的两份流程版本，分别由用户在编排时指定 LangGraph、AgentScope 担任主智能体；两者都执行工具→两个并行子任务→人工检查→产物，并验证保存/关闭/新进程恢复。记录真实根调用，不能把另一内核的父任务藏在幕后。
2. 双向委派：LangGraph 主→AgentScope 子、AgentScope 主→LangGraph 子分别检查等待输入、状态、产物和失败；同内核父子另行核对。
3. 用户画布：在两种主绑定下改连线、参数和模型策略；非法连接被拒；旧运行仍指向旧版本。
4. 历史分叉：分别拒绝原尝试、从产品声明支持的旧节点改输入重跑、比较并选定；原记录哈希不变。Agent 内部任意步骤分叉另列能力，不能混称。
5. 真实模型：小模型路由/升级、同一输入换模型、异常/预算/取消；记录质量与成本。
6. 故障注入：写入后崩溃、事件断流、子任务失败、授权撤销、模型超时。

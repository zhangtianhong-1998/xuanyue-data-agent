...
修订：2026-09-26；状态：待评审。流程执行、回溯语义和轨迹交互只在本页完整定义；字段见[核心对象](03-core-contracts.md)。

**目标用法：**用户在编排“固定程序处理输入 → 小模型选路 → 两个 Agent 分工 → 人工检查”的流程时，指定谁是主智能体以及它使用的内核，然后保存版本。运行时能看到每一步的公开记录；对结果不满意时，从已保存的节点改参数再试，旧结果仍可查看。这些是设计目标，画布和完整轨迹界面尚未实现。

按问题查阅：第 1–2 节讲如何编排步骤并控制模型成本；第 3–4 节区分流程图、实际执行和历史尝试；第 5 节讲轨迹界面；第 6 节列出两种主内核都要通过的验收步骤。

## 1. 用户如何固化成熟逻辑

工作流画布保存 WorkflowDefinition，不保存某次运行的临时状态。用户可从模板开始，连接节点、指定主智能体及其内核、设置参数和模型策略，再保存版本；Agent 可以提出草案，用户确认后成为可执行版本。运行已保存版本时，产品先检查所选内核能否完成该流程，然后按配置启动；不兼容就说明原因，不能擅自换成另一种主内核。修改主智能体须保存新版本，旧运行仍指向旧版本。节点由产品层还是所选内核的适配器调度，仍待[双主路线实验](02-kernel-selection.md#3-仍需比较的两条工作流路线)决定；这项技术选择不改变用户指定的主智能体。

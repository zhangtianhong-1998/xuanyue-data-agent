...
**目标用法：**用户保存“固定程序处理输入 → 小模型选路 → 两个 Agent 分工 → 人工检查”的流程。运行时能看到每一步的公开记录；对结果不满意时，从已保存的节点改参数再试，旧结果仍可查看。这些是设计目标，画布和完整轨迹界面尚未实现。

按问题查阅：第 1–2 节讲如何编排步骤并控制模型成本；第 3–4 节区分流程图、实际执行和历史尝试；第 5 节讲轨迹界面；第 6 节列出两种主内核都要通过的验收步骤。

## 1. 用户如何固化成熟逻辑

工作流画布保存 WorkflowDefinition，不保存某次运行的临时状态。用户可从模板开始，连接节点、设置参数和模型策略、试跑，再保存新版本；Agent 可以提出草案，用户确认后成为可执行版本。运行中的旧版本不会随画布编辑改变。用户为新任务选择 LangGraph 或 AgentScope 主智能体；工作流执行层归谁负责仍待[双主路线实验](02-kernel-selection.md#3-仍需比较的两条工作流路线)决定。同一流程在两个主绑定下必须通过共同验收，不能把 AgentScope 仅挂在 LangGraph 根图下充当“已切换主智能体”。
...
1. 固定图：对同一份定义，分别由 LangGraph、AgentScope 担任主智能体，执行工具→两个并行子任务→人工检查→产物；保存/关闭/新进程恢复。记录真实根调用，不能把另一内核的父任务藏在幕后。

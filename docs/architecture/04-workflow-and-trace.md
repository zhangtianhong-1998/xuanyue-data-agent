...
用户也可以事先编排一条**确定性工作流**，例如“核对字段 → 按已确认口径计算收入 → 超过阈值才生成异常表 → 人工复核”。这些步骤、输入输出和分支条件随版本保存，单独运行时不一定需要主 Agent。工作流里可以放模型或 Agent 节点，但它们的职责和可选分支受流程定义约束。
...
| 确定性工作流 | 节点、连线、输入输出、分支条件和版本 | 工作流按已保存规则执行；模型只能在被允许的节点和选项内判断 |
...
确定性工作流可以由用户单独启动。它的画布保存“下次怎样运行”的步骤和设置，不保存某次运行到一半的状态。用户可从模板开始，连接节点并设置参数；Agent 可以提出流程草案，但须经用户确认。工作流若包含 Agent 节点，用户在编排时决定其职责和内核；若配置了主 Agent，其选择随流程版本保存。纯固定流程可以没有主 Agent。运行前要核对流程所需的节点、模型、工具和权限；不兼容就解释原因。由产品还是某个内核调度固定节点，仍待[工作流路线实验](02-kernel-selection.md#3-独立固定流程的两条候选路线)决定。
...
4. **独立固定流程：**另用已保存的步骤验证连线、参数、模型路由、非法连接、版本和恢复。包含 Agent 节点时再分别验证内核绑定；纯固定流程不要求主 Agent。

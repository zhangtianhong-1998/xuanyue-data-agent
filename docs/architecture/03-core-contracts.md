...
远端标识按服务身份、项目、会话与分支隔离；`contextId` 只是远端关联标识，不作为权限凭据。自主任务创建时的主绑定，以及工作流版本中指定的主 Agent、子 Agent 绑定，在运行开始后固定；改变已保存流程的 Agent 配置须另存版本，不追溯改写旧运行。

分支共享的历史消息、事件和产物保持原 ID、原记录；派生会话通过获准的来源引用读取它们，只为分叉后的部分写新记录。在原会话改写后续时，`active_head_ref` 指向新分支，旧头在保留期内可切回。活动头更新须防并发覆盖；删除分支时须检查其他分支是否仍引用共同节点和产物。这里规定的是产品自有记录的存储方式，尚不能推断内核原生检查点也会物理去重。分叉示例和两种操作见[工作流与轨迹](04-workflow-and-trace.md#2-agent-运行时怎样从旧节点分叉)；具体存储后端和保留期仍待实验。

...
**目标用法：**用户上传两期销售表，系统保存查询条件和数据版本，再用同一份结果绘图。点击“东区”会形成可返回的下钻状态；报告中的数字能追到查询和原文件。这一组对象留给后续数据分析阶段，目前只是草案。

以下从首轮 BI 研究移入，作为领域协议唯一草案；D1 再评审，尚未实现。代码片段尚未编译检查。

按问题查阅：第 1 节是查询条件，第 2 节是图表配置，第 3 节是下钻与返回，第 4 节是报告结论如何找到数据来源。

...
同一产品中，由 LangGraph 或 AgentScope 运行的 Agent 都应能成为自主任务的**主 Agent**：接收目标、维护自己的执行状态、委派和观察子 Agent、汇总结果，并通过统一界面呈现。用户创建自主任务时选择主内核，任务运行后固定绑定；下一项任务可以改选，旧运行仍保留原绑定。另有可独立运行的确定性工作流；只有含 Agent 节点时才需要为这些节点指定职责和内核。混合父子至少要分别检查 LangGraph 主→AgentScope 子、AgentScope 主→LangGraph 子。

**谁担任自主任务的主 Agent，由用户创建任务时决定。** 独立确定性工作流由产品自有调度层运行，还是由框架的工作流适配器运行，是另一项待评审的技术选择。有 Agent 节点时，执行器必须遵从流程版本中保存的配置；纯固定流程可以没有主 Agent。能力检查不通过时应说明原因并停止，不得暗中改用另一种内核。AgentScope 的历史节点分叉尚未验证；两个框架能否完整执行同一固定流程另需实验。

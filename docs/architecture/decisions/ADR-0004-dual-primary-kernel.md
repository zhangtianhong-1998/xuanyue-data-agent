...
- 日期：2026-09-25。
- 修订：2026-09-26，明确由用户在编排时指定主 Agent 及内核。
...
同一产品中，由 LangGraph 或 AgentScope 运行的 Agent 都应能成为一次任务的**主 Agent**：接收目标、维护自己的执行状态、委派和观察子 Agent、汇总结果，并通过统一界面呈现。用户编排工作流时指定主 Agent 及其内核；保存后，工作流版本记录这一选择，运行该版本须遵从配置。更换主 Agent 需要保存新版本，旧运行仍绑定旧版本。没有保存工作流的临时任务，可在创建任务时选择。混合父子至少要分别检查 LangGraph 主→AgentScope 子、AgentScope 主→LangGraph 子。

**谁担任主 Agent，由用户在编排时决定。** 工作流由产品自有调度层运行，还是由所选内核的工作流适配器运行，是另一项待评审的技术选择。无论采用哪条路线，调度层都必须遵从工作流版本中保存的主 Agent 配置。能力检查不通过时应说明原因并停止，不得暗中改用另一种内核。AgentScope 的历史节点分叉尚未验证；两个框架能否完整执行同一工作流仍需实验。

...
workflow 建议先完成可编辑分析步骤、进度与中断恢复，再增加拖拽画布。首版无需让用户从空白画布搭建复杂图。React Flow 可作为编辑器；执行状态必须由后端持久层管理。
...
**首版建议：**优先提供经实现和验证的数据工具，例如导入、schema 检查、只读查询、指定统计分析、绘图配置、报告产出。若要执行模型生成的任意 Python 或 shell，则必须单独通过三系统执行隔离评审。提示词约束、路径校验和用户确认均不能宣称为 OS 沙箱。
...
## 6. workflow：计划、画布和持久执行

**事实：**React Flow 能编辑节点与连线，其保存示例恢复的是图和视口等编辑状态；计算示例演示节点数据如何在 UI 中流动。[W1][W2] 这些文档没有提供进程崩溃后的业务步骤恢复承诺。

**建议：**将三个对象分开保存：

| 对象 | 包含内容 | 修改影响 |
| --- | --- | --- |
| 分析方案 `workflow_definition` | 业务步骤、依赖、参数、条件、输入输出类型、版本 | 保存产生新版本；节点位置单独保存 |
| 执行实例 `run` | 方案版本、数据快照、权限、预算、模型与工具版本 | 开始执行后保持来源可追溯 |
| 步骤尝试 `step_attempt` | 状态、输入引用、输出引用、异常、重试次数、耗时 | 重试追加记录；不能覆盖失败历史 |

画布展示来自执行服务的状态；拖动节点不触发业务工具。分支、重试、并行、循环、人工确认都是执行语义，需要编译/校验方案后再执行。首版可只支持有限 DAG 和人工确认，暂不支持任意循环和用户代码节点。

### 执行器候选

| 路线 | 有依据的能力 | 本项目需补充 | 当前建议 |
| --- | --- | --- | --- |
| 小型本地状态机 + SQLite | 项目自建候选；尚无实现或验证结果 | 事务、租约/单实例、恢复、幂等、版本、取消、结果落盘 | 适合少量固定业务步骤；严格限制成小执行器，避免自建通用工作流平台 |
| LangGraph + 持久 checkpointer | 官方区分线程检查点与跨线程 Store；内存 saver 重启即丢失；提供持久后端 [W3][W4] | 工具副作用、数据 artifact、权限、预算、包版本迁移；SQLite 用途需按实测评审 | 若 Agent 内核选 LangGraph，优先复用检查点，不平行实现第二套 Agent 状态机 |
| Temporal | Event History + replay；Service 与 Worker 配合执行 [W5] | 服务部署与生命周期、SDK 确定性要求、升级、桌面分发、外部活动幂等 | 本地个人 MVP 暂不选；多人服务、跨机器长期任务成为真实需求时再评审 |

LangGraph 具体 node/task 恢复位置、并行失败后的 pending writes、interrupt 行为应在锁定版本上实验，不从“支持 checkpoint”推断“任何工具只执行一次”。其概览将 SQLite saver 描述为本地开发存储，详细指南则称适合实验和本地 workflow；这不能直接推导为桌面产品必须部署 PostgreSQL。个人产品可将 SQLite 列为候选，验证并发、损坏恢复、备份和升级后再决定。[W3][W4]

无论选哪条路线，都只保留一个负责调度和恢复的权威执行状态。UI、事件查询表和业务证据表可以是它的投影。未来替换执行器时，通过定义、事件和 artifact 接口迁移，不让 UI 绑定某个框架数据库。

### 恢复语义的最低要求

建议每步至少区分 `pending`、`running`、`waiting_approval`、`succeeded`、`failed`、`cancelled`、`interrupted`、`outcome_unknown`。任务不存在于内存中不代表任务已结束。

1. 开始工具前持久记录输入引用、数据版本、权限和 attempt ID。
2. 输出先写临时产物，完成后登记哈希与结果，再提交成功状态；启动时清理或核对孤立临时文件。
3. 崩溃后，已经成功并持久登记的纯计算步骤可复用；输入哈希变化就不能直接复用。
4. 对“远端已收到请求，本地尚未保存结果”的间隙，先记录结果未知。支持幂等键或远端状态查询时才据此恢复；否则提示可能重复请求或费用。
5. 对覆盖文件、发送内容或外部写操作，重试必须符合该工具的幂等或人工确认规则，不宣称 exactly-once。
6. 可恢复并不要求重算模型输出得到相同文本；需要保存可见请求、已返回结果、版本与证据引用。所有记录均排除隐藏推理。

以上是项目设计要求，未在本轮实现；不能当作框架现成保证。
...
1. 是否接受以 Electron 作为第一项打包实验，并把 Tauri 作为有明确触发条件的替代候选。
2. 首版“跨平台”是三系统都可交付，还是先验证 Mac、其他两系统在同一阶段补齐。后者只能对外称 Mac 预览版。
3. 首版退出应用后任务停止、重启恢复是否足够；若要求后台持续运行，要增加系统服务方案评审。
4. workflow 首版是否接受“模板 + 可编辑步骤 + 运行可视化”，拖拽自由编排在基本分析正确后加入。
5. 是否只开放受控分析工具；任意生成代码、第三方 Skill 脚本与通用 shell 若必须进入首版，需要追加执行隔离实验。
